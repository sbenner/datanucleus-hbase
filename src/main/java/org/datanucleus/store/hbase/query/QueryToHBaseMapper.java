...
public class QueryToHBaseMapper extends AbstractExpressionEvaluator {
    ...
    protected PrimaryDetails getFamilyColumnNameForPrimary(List<String> tuples) {
        ...
        try {
            if (!ec.getStoreManager().managesClass(cmd.getFullClassName())) {
                ec.getStoreManager().manageClasses(clr, cmd.getFullClassName());
            }

            table = ec.getStoreManager().getStoreDataForClass(candidateCmd.getFullClassName()).getTable();
        } catch (Exception ex) {
            ex.printStackTrace();
            return null;
        }
